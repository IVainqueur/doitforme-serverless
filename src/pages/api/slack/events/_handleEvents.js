...
export function setupHandlers() {
  ...
  app.message(subtype("message_deleted"), async ({ message, say }) => {
    ...
    const compositionDate = new Date(
      Number(Math.floor(message.previous_message.ts) * 1000)
    ...
    const screenshotURI = `${NIGEN_URI}?message=${encodeURIComponent(
      message.previous_message.text
    )}&username=${encodeURIComponent(userInfo.name)}&time=${encodeURIComponent(
      compositionDate
    )}&avatar=${userInfo.avatar}`;
    await say({
      text: reply
        ? ""
        : `<!channel>, <@${message.previous_message.user}> deleted their message!`,
